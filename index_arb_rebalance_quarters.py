...
num_trading_days_in_quarter = 63
...
class Arb:
    ...
    def calculate_return(self):
        ...
        quarterly_returns = log_return[1:].groupby(np.arange(len(log_return[1:])) // num_trading_days_in_quarter).mean()
    ...
    def generate_portfolios(self):
        ...
        for i in range(num_quarters):
            for _ in range(num_portfolios):
                ...
                portfolio_means.append(np.sum(qreturns.iloc[i] * w) * num_trading_days_in_quarter)
                portfolio_risks.append(np.sqrt(np.dot(w.T, np.dot(dreturns.cov() * num_trading_days_in_quarter, w))))
                sharpe_list.append(
                    (np.sum(qreturns.iloc[i] * w) * num_trading_days_in_quarter - risk_free_rate / 4) / np.sqrt(
                        np.dot(w.T, np.dot(dreturns.cov() * num_trading_days_in_quarter, w))))
    ...
    def max_function_sharpe(self):
        ...
        return sum_of_returns, sum_of_risks, (sum_of_returns - risk_free_rate * 5) / sum_of_risks#, optimal_weights, max_sharpe_stats
